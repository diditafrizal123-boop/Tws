...
function UI:GetResponsiveScale()
    ...
    local widthScale = viewport.X / 800
    local heightScale = viewport.Y / 600
    local rawScale = ClonedFunctions.mathMin(widthScale, heightScale)
    local minScale = 0.72
    local maxScale = 1

    return ClonedFunctions.mathMax(minScale, ClonedFunctions.mathMin(rawScale, maxScale))
end

function UI:ApplyScaleToWindow(window)
    ...
    Utils.SafeSet(uiScale, "Scale", scale)
end

function UI:ApplyResponsiveScale()
    if self.Main then
        self:ApplyScaleToWindow(self.Main)
    end
    for _, window in pairs(self.SubWindows) do
        self:ApplyScaleToWindow(window)
    end
end
...
function UI:BuildMain()
    ...
    local camera = Services.Workspace and Services.Workspace.CurrentCamera
    if camera then
        self.ScaleConnection = camera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
            self:ApplyResponsiveScale()
        end)
    end
end
